...
AddEventHandler('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	if Config.EnableHud then
		...
		local orgaTpl = '<div>{{orga_label}}{{grade2_label}}</div>'
		local grade2Label = ESX.PlayerData.orga.grade_label ~= ESX.PlayerData.orga.label and ESX.PlayerData.orga.grade_label or ''
		if grade2Label ~= '' then grade2Label = ' - '..grade2Label end

		ESX.UI.HUD.RegisterElement('orga', #ESX.PlayerData.accounts, 0, orgaTpl, {
			orga_label = ESX.PlayerData.orga.label,
