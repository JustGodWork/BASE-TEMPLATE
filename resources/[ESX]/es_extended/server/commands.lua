...
ESX.RegisterCommand('setorga', 'admin', function(xPlayer, args, showError)
	if ESX.DoesOrgaExist(args.orga, args.grade) then
		args.playerId.setOrga(args.orga, args.grade)
	else
		showError(_U('command_setorga_invalid'))
	end
end, true, {help = _U('command_setorga'), validate = true, arguments = {
	{name = 'playerId', help = _U('commandgeneric_playerid'), type = 'player'},
	{name = 'orga', help = _U('command_setorga_orga'), type = 'string'},
	{name = 'grade', help = _U('command_setorga_grade'), type = 'number'}
}})
...
ESX.RegisterCommand('save', 'admin', function(xPlayer, args, showError)
	...
	print(("[^2INFO^0] Sauvegarde du joueur [^1%s^0] !"):format(args.playerId.name))
