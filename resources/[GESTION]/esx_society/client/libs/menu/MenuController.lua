...
function RageUI.Navigation()
    if RageUI.CurrentMenu ~= nil then
        if RageUI.CurrentMenu() then
            local arrowonbottom = nil
            if RageUI.CurrentMenu.EnableMouse then
                SetMouseCursorActiveThisFrame()
            end
            if MFAConfig.ShowArrowOnBottom then
                arrowonbottom = RageUI.Options <= RageUI.CurrentMenu.Pagination.Total
            else
                arrowonbottom = RageUI.Options >= RageUI.CurrentMenu.Pagination.Total
            end
            if arrowonbottom then
...
function RageUI.GoBackWhenTrue()
    if RageUI.CurrentMenu ~= nil then
        local Audio = RageUI.Settings.Audio
        RageUI.PlaySound(Audio[Audio.Use].Back.audioName, Audio[Audio.Use].Back.audioRef)
        if RageUI.CurrentMenu.Parent ~= nil then
